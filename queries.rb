...
timed_session_intervals AS (
    SELECT
        scheduled_build_days.date AS build_date,
        lab_sessions.student_id,
        GREATEST(lab_sessions.time_in, scheduled_build_days.starts_at) AS interval_start,
        LEAST(lab_sessions.time_out, scheduled_build_days.ends_at) AS interval_end
    FROM scheduled_build_days
    JOIN lab_sessions
        ON lab_sessions.time_in < scheduled_build_days.ends_at
        AND lab_sessions.time_out > scheduled_build_days.starts_at
        AND NOT lab_sessions.excluded_from_total
    WHERE
        NOT ISNULL(scheduled_build_days.starts_at)
        AND NOT ISNULL(scheduled_build_days.ends_at)
        AND NOT ISNULL(lab_sessions.time_out)
),
...
def optional_build_case(build_date_ref)
  <<~SQL
    CASE
        WHEN NOT ISNULL(scheduled_build_days.date) THEN scheduled_build_days.optional
        WHEN NOT ISNULL(optional_builds.date) THEN 1
...
def build_attended_case
  <<~SQL
    CASE
        WHEN NOT ISNULL(scheduled_build_days.starts_at)
            AND NOT ISNULL(scheduled_build_days.ends_at)
        THEN
            scheduled_build_days.ends_at <= UTC_TIMESTAMP()
            AND COALESCE(timed_build_attendance.attended_seconds, 0) * 3 >=
                TIMESTAMPDIFF(SECOND, scheduled_build_days.starts_at, scheduled_build_days.ends_at) * 2
        ELSE MAX(NOT ISNULL(lab_sessions.time_in))
    END
  SQL
end
...
def build_finalized_case
  <<~SQL
    CASE
        WHEN NOT ISNULL(scheduled_build_days.starts_at)
            AND NOT ISNULL(scheduled_build_days.ends_at)
        THEN scheduled_build_days.ends_at <= UTC_TIMESTAMP()
        ELSE 1
    END
  SQL
end
...
BUILD_DAYS_QUERY = """
WITH
    lab_build_days AS (SELECT DISTINCT #{utc_to_local_date("time_in")} AS build_date FROM lab_sessions WHERE NOT excluded_from_total),
    optional_build_days AS (SELECT date AS build_date FROM optional_builds),
    sbd_dates AS (SELECT date AS build_date, optional FROM scheduled_build_days),
    all_build_days AS (
        SELECT build_date FROM lab_build_days
        UNION
        SELECT build_date FROM optional_build_days
        UNION
        SELECT build_date FROM sbd_dates
    )
SELECT
    all_build_days.build_date,
    #{optional_build_case("all_build_days.build_date")} AS optional,
    scheduled_build_days.starts_at,
    scheduled_build_days.ends_at
FROM
    all_build_days 
    LEFT JOIN optional_builds ON optional_builds.date=all_build_days.build_date
    LEFT JOIN scheduled_build_days ON scheduled_build_days.date=all_build_days.build_date
...
WITH
    ...
    lab_build_days AS (SELECT DISTINCT #{utc_to_local_date("time_in")} AS build_date FROM lab_sessions WHERE NOT excluded_from_total),
    optional_build_days AS (SELECT date AS build_date FROM optional_builds),
    sbd_dates AS (SELECT date AS build_date FROM scheduled_build_days),
    build_days AS (
        SELECT build_date FROM lab_build_days
        UNION
        SELECT build_date FROM optional_build_days
    ...
    ordered_students AS (
        ...
        FROM
            students
        LEFT JOIN counts ON students.id=counts.student_id
    )
SELECT
    ...
    CASE
        ...
        ELSE 1
    END AS required,
    MAX(NOT ISNULL(excused_sessions.date)) AS excused,
    #{not_yet_joined_case("build_days.build_date", "ordered_students.join_date")} AS not_yet_joined,
    MAX(lab_sessions.id) AS session_id,
    scheduled_build_days.starts_at,
    scheduled_build_days.ends_at
    FROM
        build_days CROSS JOIN ordered_students
        LEFT JOIN optional_builds ON optional_builds.date=build_days.build_date
        LEFT JOIN scheduled_build_days ON scheduled_build_days.date=build_days.build_date
    ...
    LEFT JOIN lab_sessions ON #{utc_to_local_date("lab_sessions.time_in")} = build_days.build_date
        AND lab_sessions.student_id=ordered_students.student_id
        AND NOT lab_sessions.excluded_from_total
    LEFT JOIN excused_sessions ON excused_sessions.student_id=ordered_students.student_id
        AND excused_sessions.date=build_days.build_date
GROUP BY
    build_days.build_date,
    ordered_students.student_id,
    ordered_students.join_date,
    ordered_students.sessions_attended_count,
    optional_builds.date,
    scheduled_build_days.date,
    scheduled_build_days.optional,
    scheduled_build_days.starts_at,
    scheduled_build_days.ends_at,
    ...
    ordered_students AS (
        WITH counts AS (
            SELECT
                ...
                lab_sessions.student_id
            FROM lab_sessions
            JOIN filtered_build_days ON #{utc_to_local_date("lab_sessions.time_in")} = filtered_build_days.build_date
            WHERE NOT lab_sessions.excluded_from_total
            GROUP BY lab_sessions.student_id
        )
        SELECT
            id as student_id,
            join_date,
...
SELECT
    ...
    MAX(lab_sessions.id) AS session_id,
    MAX(CASE
        WHEN NOT ISNULL(lab_sessions.time_in)
            AND ISNULL(lab_sessions.time_out)
        THEN 1
        ELSE 0
    END) AS signed_in,
    scheduled_build_days.starts_at,
    ...
    FROM
        ...
        LEFT JOIN optional_builds ON optional_builds.date=filtered_build_days.build_date
        LEFT JOIN scheduled_build_days ON scheduled_build_days.date=filtered_build_days.build_date
        LEFT JOIN timed_build_attendance ON timed_build_attendance.build_date=filtered_build_days.build_date
            AND timed_build_attendance.student_id=ordered_students.student_id
    LEFT JOIN lab_sessions ON #{utc_to_local_date("lab_sessions.time_in")} = filtered_build_days.build_date
        AND lab_sessions.student_id=ordered_students.student_id
        AND NOT lab_sessions.excluded_from_total
    LEFT JOIN excused_sessions ON excused_sessions.student_id=ordered_students.student_id
        AND excused_sessions.date=filtered_build_days.build_date
GROUP BY
    filtered_build_days.build_date,
    ordered_students.student_id,
    ordered_students.join_date,
...
WITH build_info AS (
    ...
    FROM
        build_days CROSS JOIN students
        LEFT JOIN optional_builds ON optional_builds.date=build_days.build_date
        LEFT JOIN scheduled_build_days ON scheduled_build_days.date=build_days.build_date
        LEFT JOIN timed_build_attendance ON timed_build_attendance.build_date=build_days.build_date
            AND timed_build_attendance.student_id=students.id
        LEFT JOIN lab_sessions ON #{utc_to_local_date("lab_sessions.time_in")} = build_days.build_date
            AND lab_sessions.student_id=students.id
            AND NOT lab_sessions.excluded_from_total
        LEFT JOIN excused_sessions ON excused_sessions.student_id=students.id
            AND excused_sessions.date=build_days.build_date
        GROUP BY
            build_days.build_date,
            students.id,
...
WITH build_info AS (
    ...
    FROM
        filtered_build_days CROSS JOIN students
        LEFT JOIN optional_builds ON optional_builds.date=filtered_build_days.build_date
        LEFT JOIN scheduled_build_days ON scheduled_build_days.date=filtered_build_days.build_date
        LEFT JOIN timed_build_attendance ON timed_build_attendance.build_date=filtered_build_days.build_date
            AND timed_build_attendance.student_id=students.id
...
WITH build_info AS (
    ...
    FROM
        ...
        LEFT JOIN lab_sessions ON #{utc_to_local_date("lab_sessions.time_in")} = filtered_build_days.build_date
            AND lab_sessions.student_id = ?
            AND NOT lab_sessions.excluded_from_total
        LEFT JOIN excused_sessions ON excused_sessions.student_id = ?
            AND excused_sessions.date=filtered_build_days.build_date
        LEFT JOIN students ON students.id = ?
        GROUP BY
            filtered_build_days.build_date,
            students.join_date,
            optional_builds.date,
