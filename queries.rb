...
BUILD_DAYS_QUERY = """
WITH
    lab_build_days AS (SELECT DISTINCT DATE(CONVERT_TZ(time_in, '+00:00', '#{USER_TIME_ZONE}')) AS build_date FROM cheesy_frc_hours.lab_sessions WHERE NOT excluded_from_total),
...
SELECT DISTINCT
    ...
    LEFT JOIN cheesy_frc_hours.lab_sessions ON DATE(CONVERT_TZ(cheesy_frc_hours.lab_sessions.time_in, '+00:00', '#{USER_TIME_ZONE}')) = build_days.build_date
...
WITH
    ...
    ordered_students AS (
        WITH counts AS (
            ...
            FROM cheesy_frc_hours.lab_sessions
            JOIN filtered_build_days ON DATE(CONVERT_TZ(cheesy_frc_hours.lab_sessions.time_in, '+00:00', '#{USER_TIME_ZONE}')) = filtered_build_days.build_date
...
SELECT DISTINCT
    ...
    LEFT JOIN cheesy_frc_hours.lab_sessions ON DATE(CONVERT_TZ(cheesy_frc_hours.lab_sessions.time_in, '+00:00', '#{USER_TIME_ZONE}')) = filtered_build_days.build_date
        AND cheesy_frc_hours.lab_sessions.student_id=ordered_students.student_id
        AND NOT cheesy_frc_hours.lab_sessions.excluded_from_total
    LEFT JOIN cheesy_frc_hours.excused_sessions ON cheesy_frc_hours.excused_sessions.student_id=ordered_students.student_id
